...
export const DeleteCinemaDialog: React.FC<Props> = ({
  isOpen,
  cinema,
  onClose,
  onConfirm,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-slate-950 border border-slate-800 rounded-2xl w-full max-w-md p-6 shadow-2xl">
        ...
        <h3 className="text-base font-bold text-white">Delete Cinema Branch?</h3>
        <p className="text-xs text-slate-400 mt-2 leading-relaxed">
          Are you sure you want to delete <span className="text-white font-semibold">{cinema.name}</span> in {cinema.city}?
        </p>

        <div className="mt-3 p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-[11px] text-red-400 leading-relaxed">
          <strong>Warning:</strong> This action will permanently remove this cinema, including its {cinema.seats_count} configured seats and any associated showtimes or bookings.
        </div>
        ...
        <div className="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-slate-800">
          ...
          >
            ...
            <span>{isLoading ? 'Deleting...' : 'Delete Branch'}</span>
          </button>
        </div>
      </div>
    </div>
